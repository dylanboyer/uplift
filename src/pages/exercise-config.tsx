...
export default function ExerciseConfig() {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Pick an exercise to make a goal! Or create a new one</h1>
        ...
        {exercises.length > 0 ? (
          exercises.map((exercise) => (
            <button
              ...
              onClick={() => {
                setSelectedExercise({ name: exercise.name, id: exercise.e_id });
      ...
      {isNewExercise ? (
        ...
        <EditExerciseForm exerciseId={selectedExercise.id} exerciseName={selectedExercise.name} />
      ) : null}
    </div>
  );
}
